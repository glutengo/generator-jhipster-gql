...
describe('Post Controller', () => {
  ...
  it('/POST create posts', async () => {
    const createdEntity: PostDTO = (await request(app.getHttpServer()).post('/api/posts').send(entityMock).expect(201)).body;

    expect(createdEntity).toEqual(entityMock);
  });

  it('/PUT update posts', async () => {
    const updatedEntity: PostDTO = (await request(app.getHttpServer()).put('/api/posts').send(entityMock).expect(201)).body;
